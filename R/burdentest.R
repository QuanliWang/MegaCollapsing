burden.test <- function(sample.list, syn.mat, non.syn.mat, X = NULL, Y = NULL, ncores = 1) {
  ...
  cleanY <- Y
  cleanY[is.na(cleanY)] <- -1
  median.syn.case <- median(rowSums(syn.mat[,cleanY == 1]))
  median.syn.control <- median(rowSums(syn.mat[,cleanY == 0]))
  median.non.syn.case <- median(rowSums(non.syn.mat[,cleanY == 1]))
  median.non.syn.control <- median(rowSums(non.syn.mat[,cleanY == 0]))
  ...
  return(list(p_values = p_values, betas = betas, median.syn.case = median.syn.case,
              ...
              median.non.syn.control = median.non.syn.control,mega.gene.names = mega.gene.names))
}


burden.test.transposed <- function(sample.list, syn.mat, non.syn.mat, X = NULL, Y = NULL, ncores = 1) {
  ...
  cleanY <- Y
  cleanY[is.na(cleanY)] <- -1
  median.syn.case <- median(colSums(syn.mat[cleanY == 1,]))
  median.syn.control <- median(colSums(syn.mat[cleanY == 0,]))
  median.non.syn.case <- median(colSums(non.syn.mat[cleanY == 1,]))
  median.non.syn.control <- median(colSums(non.syn.mat[cleanY == 0,]))
  ...
  p <- dim(data.base)[2] + 2
  mega.gene.names <- colnames(syn.mat)
